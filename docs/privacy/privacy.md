...
**Registro dei consensi** (`V27`). Tabella in sola aggiunta: la revoca inserisce
...
Rimedio: `V28` aggiunge le coordinate, che il percorso di scrittura ora
...
Questo branch era stato scritto per `V22`–`V24`, contando su `massi_sprint_10`
fermo a `V21`. Al momento del merge quel ramo era invece arrivato a `V26`, con
un proprio `V22`–`V24`: due migration con lo stesso numero bloccano Flyway
all'avvio, quindi il set privacy è stato spostato a **`V27`, `V28`, `V29`**.
...
**Migration** — `V1`→`V29` applicate in sequenza da Flyway attraverso
