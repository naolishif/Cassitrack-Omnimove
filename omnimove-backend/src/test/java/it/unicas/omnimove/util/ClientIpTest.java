...
/**
 * The address the audit trail, the consent ledger and the rate limiter all key
 * on. Two things have to hold at once: behind our own proxy the visitor's real
 * address must come through, and a caller who sends X-Forwarded-For himself
 * must not be able to choose what gets recorded — or whose rate-limit bucket
 * he spends.
 */
class ClientIpTest {
